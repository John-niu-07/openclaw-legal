...
1. 检查 `../../../../../ai-governance-legal/CLAUDE.md` 是否已存在。如果存在且用户未传递 `--redo` → "AI治理实践配置已存在于 [路径]。使用 `--redo` 重新运行。"
2. 运行以下访谈。一次进行一个部分。
3. 选项后附 `(✓)` 标注推荐默认值。
...
路径：`[绝对路径]`
示例：`../../../../../ai-governance-legal/outputs/`
...
A. 是，隐私实践已配置（路径：`../../../../../privacy-legal/CLAUDE.md`）
...
访谈完成后，将所有回答编译为 `../../../../../ai-governance-legal/CLAUDE.md`，结构如下：
...
告知用户：
> "AI治理实践配置已写入 `../../../../../ai-governance-legal/CLAUDE.md`。
>
> **下一步建议：**
