...
>
> **来源溯源。** 标记每条引注和法规事项的来源：`[中国政府网]`、`[<监管机构>官网]`、`[wkinfo浏览器自动化（browser-automation）]`、`[法律数据库]`，或具体MCP工具名称（对于通过连接器获取的事项）；`[联网检索 — 需复核]`（对于通过联网搜索获取的事项）；`[模型知识 — 需验证]`（对于从模型训练数据中浮现的事项）；`[用户提供]`（对于手动粘贴的事项）。标记为 `需验证` 的事项具有比工具获取事项更高的编造风险，应首先检查。绝不要剥离或折叠标签。
...
**期间：** [上次检查] 至 [现在]
**已检查的动态源：** [列出活跃的层级——如"中国政府网、网信办官网、wkinfo浏览器自动化（browser-automation）"]
**发现事项：** [N] 总计
...
**依赖前核实引注。** 此处的法规引注由AI生成，未经对照一手来源核实。在对上述任何法规、规章、指导意见或执法行动采取行动之前，通过法律研究工具（wkinfo浏览器自动化（browser-automation）、你的律所研究平台或发布机构的官方网站）确认其准确性、生效日期和当前状态。AI生成的法规引注有时是虚构、引用错误或过时的。每个事项上的来源标签（例如 `[中国政府网]`、`[联网检索 — 需复核]`）标明其出处；`需验证` 标签具有更高的编造风险，应首先检查。
```
