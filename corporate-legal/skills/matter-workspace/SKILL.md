...
- `/corporate-legal:matter-workspace close <简称>` — 归档事项（移至 `../../../../../corporate-legal/matters/_archived/`，绝不删除）
- `/corporate-legal:matter-workspace none` — 脱离任何活跃事项，仅以实务级工作

## 指令

1. 读取 `../../../../../corporate-legal/CLAUDE.md` ——确认 `## 事项工作区` 部分已填充。如果 `Enabled` 为 `✗`，告知用户："事项工作区已关闭——你的配置为企业法务，仅服务一家公司，因此插件自动在实务级上下文下工作。如果你实际为多家客户工作，重新运行 `/corporate-legal:cold-start-interview --redo` 并选择私人执业设置。否则，你完全不需要 `/matter-workspace`。"不要报错——对于企业法务用户，关闭状态是预期状态。
2. 使用以下工作流。
3. 按 `$ARGUMENTS` 的第一个 token 分发：
   - `new` → 运行信息采集访谈，写入 `../../../../../corporate-legal/matters/<简称>/matter.md`，初始化 `history.md` 和 `notes.md`。
   - `list` → 枚举 `../../../../../corporate-legal/matters/*/matter.md`，打印表格，标记活跃事项。
   - `switch` → 更新实务级 CLAUDE.md 中的 `活跃事项：` 行。
   - `close` → 将 `../../../../../corporate-legal/matters/<简称>/` 移至 `../../../../../corporate-legal/matters/_archived/<简称>/`，在 `history.md` 中记录关闭日期。
