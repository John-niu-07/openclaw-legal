...
从股权收购协议中提取"重大合同"的定义——以收购协议的定义为准。交易结构差异（股权转让 vs. 资产转让 vs. 合并）会改变各触发条件的解释，而受监管行业的叠加层（医疗、国防、金融服务、电信、政府采购）可能增加收购协议之外的同意要求。如果交易涉及上述任何叠加层，检索适用的反垄断法、外商投资安全审查等规则并引用控制性规定 `[wkinfo浏览器自动化]`。
