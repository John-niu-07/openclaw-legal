...
> 本输出中的引用由 AI 模型生成，尚未与主源核对。依赖任何法规、指引或执法行动前，请对照法律检索工具（如 wkinfo浏览器自动化（browser-automation） 或发布机关官网）核查准确性和现行效力。AI 生成的引用可能存在编造或引用错误。每条引用上的来源标签（如 `[联网检索 — 需复核]`）显示其来源；带 `验证` 的标签造假风险更高，应优先核对。
