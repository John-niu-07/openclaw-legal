...
>
> **来源标注。** 为方案中的每个引用——最终工资规则、未休年休假规则、通知要求、经济补偿规则、解除类型对应法条——标注来源：`[wkinfo浏览器自动化]`用于通过检索连接器获取的引用；`[联网检索——需复核]`用于联网搜索引用；`[模型知识——需验证]`用于模型知识回忆的引用；`[用户提供]`用于用户提供的引用。标注`需验证`的引用具有较高的编造风险，应首先核验。不得删除或压缩标注。
