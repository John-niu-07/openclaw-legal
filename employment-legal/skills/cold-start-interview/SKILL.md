...
1. 检查 `../../../../../employment-legal/CLAUDE.md`。如果 `--check-integrations`，跳过访谈——仅重新运行 Part 0 `有什么连接？`检查并重写配置路径下的 `## Available integrations` 表格。探测时：只有 MCP 工具调用实际成功才报告 ✓。配置但未测试的连接器应标记 ⚪ 并附一行确认方法。绝不基于 `.mcp.json` 声明报告 ✓——这会误导用户以为某事已接通而实际没有。
...
6. 写入 `../../../../../employment-legal/CLAUDE.md`，根据需要创建父目录。
...
> "完成。你的配置在 `../../../../../employment-legal/CLAUDE.md`——一个你可以直接阅读和编辑的纯文本文件。你回答的任何内容都可以修改：
>
> - 直接编辑文件进行快速修改
